...
from app.api.dependencies.auth import get_current_active_user, log_audit
...
async def create_completion(
    ...
    if not rate_limit_ok:
        ...
        await log_audit(
            ...
            description=f"AIトークン制限超過: {rate_limit_msg}",
            user_id=current_user.id,
    ...
    if not await ai_service.validate_model_access(current_user, model):
        await log_audit(
            ...
            description=f"未許可AIモデルへのアクセス試行: {model}",
            user_id=current_user.id,
            username=current_user.username,
            user_role=current_user.role.value,
